    ...
    fn check_encounter(&mut self) {
        if  self.map[self.player.y][self.player.x] == 'M' {
                ...
                if let Some(monster) = self.monsters.get_mut(0) {
                    ...
                    if !monster_alive {
                        self.map[self.player.y][self.player.x] = '.';
                        // 如果怪物被击败，可以从 `self.monsters` 中移除它,并获得它的装备
                        let carried_item = monster.carried_item.clone(); // 如果 `carried_item` 是可克隆的
                        self.player.items.push(carried_item);
    ...
    fn battle(
        ...
        while player_health > 0 && monster_health > 0 {
            ...
            if monster_health <= 0 {
                ...
                println!("Your left health is: {}", player.health);
                return (true, false);
            }
